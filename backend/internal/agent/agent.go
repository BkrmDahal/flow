...
func buildToolGuidance(disabledSet map[string]bool) string {
	...
	sb.WriteString("- **Acting across apps (macOS)**: When a task needs you to drive another app — open or read a page, create a draft, fill a document, control a window — you can use `osascript` (AppleScript/JavaScript for Automation) or `open` via run_bash. The user is asked to approve commands before they run, so prefer one clear command per step over many small ones. Read the current screen context already provided before re-fetching it.\n")
